...
function Card({ item, type, onSelect }) {
  const { ref, focused } = useFocusable({
    ...
    onFocus: (layout) => { const n = layout && layout.node; if (n && n.scrollIntoView) n.scrollIntoView({ block: "nearest", behavior: "smooth" }); },
  });
  ...
  return (
    <div ref={ref} onClick={() => onSelect(item)} style={{ cursor: "default" }}>
      <div style={{
        width: "100%", aspectRatio: "1", borderRadius: round ? "50%" : 14, overflow: "hidden",
      ...
      <div style={{ marginTop: 12, color: "#fff", fontSize: 20, fontWeight: 600, textAlign: round ? "center" : "left", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{title}</div>
      <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 16, textAlign: round ? "center" : "left", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minHeight: "1.2em" }}>{sub || " "}</div>
    </div>
  );
}
...
export function Browse({ type, chrome, onSelect }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}${ENDPOINT[type]}`)
      .then(r => r.json())
      .then(d => { if (!cancelled) setItems(d[LISTKEY[type]] || []); })
  ...
  useEffect(() => {
    if (items && items.length) { const t = setTimeout(() => focusSelf(), 0); return () => clearTimeout(t); }
  }, [items, focusSelf]);
