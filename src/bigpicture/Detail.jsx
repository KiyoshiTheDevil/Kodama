...
function ActionBtn({ icon, label, onPress, disabled }) {
  const { ref, focused } = useFocusable({ onEnterPress: () => { if (!disabled) onPress(); }, onFocus: (l) => scrollPageTop(l && l.node) });
  return (
    <div ref={ref} onClick={() => { if (!disabled) onPress(); }} style={{
      display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
      padding: "14px 20px", borderRadius: 14, cursor: "default",
      opacity: disabled ? 0.4 : 1,
      color: focused ? "#0a0a0f" : "#fff", fontSize: 19, fontWeight: 700,
...
export function Detail({ type, item, onPlayed }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setTracks([]); setLoading(true);
    if (isAlbum) {
      fetch(`${API}/album/${item.browseId}`)
        .then(r => r.json())
        .then(d => { if (cancelled) return; setTitle(d.title || item.title || ""); if (d.thumbnail) setCover(d.thumbnail); setTracks(d.tracks || []); setLoading(false); })
    ...
    es.onmessage = (e) => {
      ...
      if (cancelled) return;
      if (msg.type === "header") { setTitle(msg.title || item.title || ""); if (msg.thumbnail) setCover(msg.thumbnail); }
      else if (msg.type === "tracks") setTracks(t => [...t, ...msg.tracks]);
      else if (msg.type === "done" || msg.type === "error") { setLoading(false); es.close(); }
    };
    es.onerror = () => { if (!cancelled) setLoading(false); es.close(); };
  ...
  const playFrom = (track) => { const list = tracks.filter(t => t.videoId); sendPlay(track, list); onPlayed && onPlayed(); };
  const playAll = () => { const list = tracks.filter(t => t.videoId); if (list.length) { sendPlay(list[0], list); onPlayed && onPlayed(); } };
  const playShuffle = () => {
    const list = tracks.filter(t => t.videoId);
    if (!list.length) return;
    for (let i = list.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [list[i], list[j]] = [list[j], list[i]]; }
    sendPlay(list[0], list); onPlayed && onPlayed();
  };
  ...
  return (
    <FocusContext.Provider value={focusKey}>
      <div ref={ref} style={{ minHeight: "100%", padding: "6vh 6vw", background: "radial-gradient(120% 80% at 50% -10%, #241033, #0a0a0f 60%)", display: "flex", gap: 48, alignItems: "flex-start" }}>
        <div style={{ width: 340, flexShrink: 0 }}>
          <div style={{ width: 340, height: 340, borderRadius: 18, overflow: "hidden", background: "linear-gradient(135deg,#2a1535,#17091f)", boxShadow: "0 18px 50px rgba(0,0,0,.5)" }}>
            {cover ? <img src={thumbHi(cover)} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /> : null}
          </div>
