...
export function GamepadTest() {
  ...
  useEffect(() => {
    const onKey = (e) => { if (e.key === "F9") { e.preventDefault(); setVisible(v => !v); } };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, []);

  useEffect(() => {
    const log = (msg) => setEvents(ev => [`${new Date().toLocaleTimeString()}  ${msg}`, ...ev].slice(0, 8));
    ...
    return () => { window.removeEventListener("gamepadconnected", onConn); window.removeEventListener("gamepaddisconnected", onDisc); };
  }, []);
  ...
  useEffect(() => {
    ...
    const poll = () => {
      ...
      for (const g of gp) {
        if (!g) continue;
        snap.push({
          index: g.index, id: g.id, mapping: g.mapping,
  ...
  return (
    <div style={{
      ...
      background: "rgba(6,6,10,0.96)", color: "#eee",
      font: "13px/1.5 ui-monospace, monospace", padding: 24, overflow: "auto",
    }}>
      ...
      <div style={{ marginBottom: 16, padding: 12, borderRadius: 8, background: "rgba(255,255,255,0.05)" }}>
        ...
        <div style={{ opacity: 0.65 }}>Browsers only expose a gamepad after its first button press.</div>
      </div>

      {pads.map(p => (
        <div key={p.index} style={{ marginBottom: 20, padding: 14, borderRadius: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(224,64,251,0.3)" }}>
          <div style={{ fontWeight: 700, color: "#e040fb", marginBottom: 8 }}>[{p.index}] {p.id} <span style={{ opacity: 0.5, fontWeight: 400 }}>· mapping: {p.mapping || "(none)"}</span></div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            {p.axes.map((a, i) => (
              <span key={i} style={{ color: Math.abs(a) > 0.15 ? "#e040fb" : "#aaa" }}>ax{i}: {a.toFixed(2)}</span>
            ))}
          </div>
        </div>
      ))}
