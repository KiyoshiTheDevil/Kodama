...
import { APP_ICON_DEFAULT, APP_ICON_GROUPS } from "./app-icons.js";
...
export function SettingsPanel({ onClose, onSectionChange, accent, onAccentChange, accentCustom, onAccentReset, accentDynamic, onAccentDynamicChange, accentSat, onAccentSatChange, accentLight, onAccentLightChange, appIcon = APP_ICON_DEFAULT, onAppIconChange,
  ...
  // The catalogue itself lives in the store window. All this panel needs is to redraw the picker
  // when something is installed or removed over there, because allThemes() is read during render.
  const [, setThemeTick] = useState(0);
  useEffect(() => onThemesChanged(() => setThemeTick(n => n + 1)), []);
          ...
          <div ref={contentScrollRef} key={tab} className="scrollable" style={{ flex: 1, overflowY: "auto", padding: "8px max(32px, calc((100% - 760px) / 2)) 32px", animation: anim ? "fadeSlideIn 0.22s cubic-bezier(0.4,0,0.2,1)" : "none" }}>
            ...
            {tab === "visualizer" && (
              <>
                ...
                <SectionLabel>{t("vizSecShape")}</SectionLabel>
                ...
                <SectionLabel>{t("vizSecBars")}</SectionLabel>
                ...
                <SectionLabel>{t("vizSecDynamics")}</SectionLabel>
                ...
                <SectionLabel>{t("vizSecColour")}</SectionLabel>
            ...
            {tab === "darstellung" && (
              <>
                ...
                <SettingRow label={t("speedDial")} description={t("speedDialDesc")} icon={<i className="fa-solid fa-grid-2" style={{ fontSize: 15 }} aria-hidden="true" />}>
                  <Toggle value={showSpeedDial} onChange={onSpeedDialChange} />
                </SettingRow>
            ...
            {tab === "wiedergabe" && (
              <>
                ...
                <SectionLabel>{t("pbSecVideo")}</SectionLabel>
                ...
                <SectionLabel>{t("pbSecPrivacy")}</SectionLabel>
            ...
            {tab === "connections" && (
              <>
                ...
                {discordRpc && (
                  <SettingRow label={t("discordClearOnPause")} description={t("discordClearOnPauseDesc")} icon={<Pause size={15} />}>
                    <Toggle value={discordClearOnPause} onChange={onDiscordClearOnPauseChange} />
                  </SettingRow>
                )}
                {discordRpc && (
                ...
                <SectionLabel>Last.fm</SectionLabel>
                <LastfmRow />

                <SectionAnchor id="conn-ytmusic" />
                <SectionLabel>YouTube Music</SectionLabel>
                ...
                <SectionLabel>{t("remoteControl")}</SectionLabel>
